...
txt=f.readlines()
f.close()
forbidden_country=['Soviet Union','Yugoslavia','West Germany','East Germany','Â',' ','Mixed team','Russia']
for line in txt:
...
sns.barplot(x=progress_gold['Gold_Progress'].head(10), y='NOC', data=progress_gold.head(10), palette='Blues_r')
...
# 金牌数退步最多的国家
plt.figure(figsize=(12, 8))
sns.barplot(x=decline_gold['Gold_Progress'].head(10), y='NOC', data=decline_gold.head(10), palette='Reds_r')
...
# 总奖牌数进步最多的国家
plt.figure(figsize=(12, 8))
sns.barplot(x=progress_total['Total_Progress'].head(10), y='NOC', data=progress_total.head(10), palette='Blues_r')
plt.title('Top 10 Countries Likely to Improve in Total Medals by 2028')
plt.xlabel('Total Medal Progress')
plt.ylabel('Country')
plt.tight_layout()
...
# 总奖牌数退步最多的国家
plt.figure(figsize=(12, 8))
sns.barplot(x=decline_total['Total_Progress'].head(10), y='NOC', data=decline_total.head(10), palette='Reds_r')
